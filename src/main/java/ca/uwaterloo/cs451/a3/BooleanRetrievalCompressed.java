...
public class BooleanRetrievalCompressed extends Configured implements Tool {
  ...
  private void initialize(String indexPath, String collectionPath, FileSystem fs) throws IOException {
   ...
   // for (FileStatus file : status) {
    for (int i=0; i < status.length; i++) {
     ...
     // if (file.getPath().toString().contains("SUCCESS")) continue;
      index[count] = new MapFile.Reader(status[i].getPath(), fs.getConf());
      count++;
    }
   // }
  ...
  private ArrayListWritable<PairOfInts> fetchPostings(String term) throws IOException {
    ...
    DataInputStream dinStream = new DataInputStream(byteStream);

    int docno = 0;
    int df = WritableUtils.readVInt(dinStream);

    ArrayListWritable<PairOfInts> postings= new ArrayListWritable<PairOfInts>();

    for (int f = 0; f < df; f++){
      int gap= WritableUtils.readVInt(dinStream);
      int tf = WritableUtils.readVInt(dinStream);
    ...
    //PairOfWritables<IntWritable, ArrayListWritable<PairOfInts>> postings = readPostings(value);
