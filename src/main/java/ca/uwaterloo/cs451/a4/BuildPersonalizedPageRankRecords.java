...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static final String SRC_NODES = "node.source";
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
      ...
      String[] srcs1 = context.getConfiguration().getStrings(SRC_NODES, "");
      String source;
      for(int i = 0; i < srcs1.length; i++){
        source = srcs1[i];
      
        srcs.add(Integer.valueOf(source));
      }
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      //setting log probabilities
      float[] pgRank = new float[srcs.size()];

      for(int i = 0; i < srcs.size(); i++){
        pgRank[i] = (srcs.get(i) == nid.get() ? (float) StrictMath.log(1): (float) StrictMath.log(0));
				}

      node.setPageRank(new ArrayListOfFloatsWritable(pgRank));
  ...
  public int run(String[] args) throws Exception {
    ...
    conf.setStrings(SRC_NODES, sources);
