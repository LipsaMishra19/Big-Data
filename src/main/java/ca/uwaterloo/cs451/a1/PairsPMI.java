...
/**
*Implementation of Pointwise Mutual Information (PMI) - Function of two evets X and Y.
*PMI(X,Y) = Log(P(X,Y)/(P(X).P(Y))
*Class: PairsPMI 
*Output: The key should be a co-occurring pair, e.g., (X,Y), and the value should be a pair (PMI, count) denoting its PMI and co-occurrence count.
**/

//Define class Pairs PMI:
...
// First Reducer: Sums up all the counts and Emits (WORD, SUM)
  ...
  // Second Mapper: Emits every word pair in a line except self pairs
  private static final class SecondPMIMapper extends Mapper<LongWritable, Text, PairOfStrings, IntWritable> {
...
//Combiner: Sums up every pair of words
  private static final class PMICombiner extends
...
//Second Reducer - To calculate the PMI and emits output as (PAIRS(X,Y),(PMI, Count))
  private static class SecondPMIReducer extends Reducer<PairOfStrings, IntWritable, PairOfStrings, PairOfFloatInt> {
    ...
    public void reduce(PairOfStrings pair, Iterable<IntWritable> values, Context context ) 
      ...
      if (sumOfPair >= threshold){
        ...
        // PMI Calculation:
        double probPairXY = (double)sumOfPair / (double)linesCount;
  ...
  private static final class Args {
    ...
    // @Option(name = "-disable-combiner", metaVar = "[String]", usage = "disable combiner")
  ...
  public int run(String[] argv) throws Exception {
    ...
    jobPMI.setJobName(PairsPMI.class.getSimpleName());
    ...
    jobPMI.setOutputValueClass(PairOfFloatInt.class);
